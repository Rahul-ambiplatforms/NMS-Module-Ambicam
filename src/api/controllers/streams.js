  ...
  this.sessions.forEach(function(session, id) {
    if (session.isStarting) {
      ...
      case session.isPublishing: {
        ...
        const elapsedTimeSeconds = (currentTime - session.connectTime) / 1000;
        _.setWith(stats, [app, stream, 'publisher'], {
          ...
          video: session.videoCodec > 0 ? {
            bitrate: `${calculateBitrate(session.socket.bytesRead, elapsedTimeSeconds)} Kb/s`,
      ...
      function calculateBitrate(bytes, elapsedTimeSeconds) {
        const bits = bytes * 8;
        const bitrate = bits / elapsedTimeSeconds / 1000; // in kilobits per second
